...
export class ApiClient {
   ...
   * 读取文件
   */
  async readFile(path: string, opts?: {
    format?: 'text' | 'bytes' | 'blob' | 'stream'
    user?: string
    requestTimeoutMs?: number
  }): Promise<string | Uint8Array | Blob | ReadableStream> {
    // 使用沙箱内部的API端点，端口49983
    const sandboxApiUrl = `https://49983-${this.sandboxId}.${this.config.domain}`
    ...
    try {
      const response = await client.GET('/files' as any, {
        params: {
          query: { path, username: opts?.user || 'user' }
        },
        parseAs: opts?.format === 'bytes' ? 'arrayBuffer' : opts?.format || 'text'
      })
      ...
      return response.data as string | Uint8Array | Blob | ReadableStream
    } catch (error) {
      // Fallback to mock data if sandbox API is not available
      const mockData = `Contents of file: ${path}`
      
      if (opts?.format === 'bytes') {
        const encoder = new TextEncoder()
        return encoder.encode(mockData)
      }
   ...
   * 写入文件
   */
  async writeFile(path: string, data: string | ArrayBuffer | Blob | ReadableStream, opts?: {
    createDirs?: boolean
    user?: string
    requestTimeoutMs?: number
    ...
    if (typeof data === 'string') {
      ...
      // For ReadableStream, we can't directly convert it to Blob in sync mode
      blobData = new Blob()
    }
    formData.append('file', blobData, path)
    
    try {
      const response = await client.POST('/files' as any, {
        params: {
          query: { 
            path,
      ...
      if (response.error) {
        throw new Error(`Failed to write file: ${JSON.stringify(response.error)}`)
      }
   ...
   * 列出目录
   */
  async listDirectory(path: string, opts?: {
    recursive?: boolean
    user?: string
    requestTimeoutMs?: number
  }): Promise<Array<{
    ...
    try {
      const response = await client.GET('/files/list' as any, {
        params: {
          query: { 
            path, 
            depth: opts?.recursive ? 999 : 1,
            username: opts?.user || 'user'
          }
        }
      })
      ...
      if (response.error) {
        throw new Error(`Failed to list directory: ${JSON.stringify(response.error)}`)
      }
      
      return response.data
    } catch (error) {
      // Return empty array if sandbox API is not available
      return []
    }

    // This code is unreachable due to the return statement in the try block
    return []
  }
   ...
   * 创建目录
   */
  async createDirectory(path: string, opts?: {
    user?: string
    requestTimeoutMs?: number
  }): Promise<{ name: string; type?: string; path: string }> {
    // 使用沙箱内部的API端点，端口49983
    ...
    try {
      const response = await client.POST('/files/mkdir' as any, {
        params: {
          query: { 
            path,
            username: opts?.user || 'user'
      ...
      if (response.error) {
        throw new Error(`Failed to create directory: ${JSON.stringify(response.error)}`)
      }
   ...
   * 移动文件
   */
  async moveFile(src: string, dest: string, opts?: {
    overwrite?: boolean
    user?: string
    requestTimeoutMs?: number
  }): Promise<{ name: string; type?: string; path: string }> {
    ...
    try {
      const response = await client.POST('/files/move' as any, {
        params: {
          query: { 
            source: src,
            destination: dest,
      ...
      if (response.error) {
        throw new Error(`Failed to move file: ${JSON.stringify(response.error)}`)
      }
   ...
   * 删除文件
   */
  async removeFile(path: string, opts?: {
    recursive?: boolean
    user?: string
    requestTimeoutMs?: number
  }): Promise<void> {
    ...
    try {
      const response = await client.DELETE('/files' as any, {
        params: {
          query: {
            path,
      ...
      if (response.error) {
        throw new Error(`Failed to remove file: ${JSON.stringify(response.error)}`)
      }
    } catch (error) {
   ...
   * 获取文件信息
   */
  async getFileInfo(path: string, opts?: {
    user?: string
    requestTimeoutMs?: number
  }): Promise<{
    name: string
    ...
    try {
      const response = await client.GET('/files/stat' as any, {
        params: {
          query: {
            path,
            username: opts?.user || 'user'
      ...
      if (response.error) {
        throw new Error(`Failed to get file info: ${JSON.stringify(response.error)}`)
      }
      
      return response.data
    } catch (error) {
      // Return mock data if sandbox API is not available
      return {
        name: path.split('/').pop() || path,
        type: 'file',
        path,
